/* ============================================================
   WordLab 초등 · 메인 앱 부트스트랩 (150단어 · 이모지 · SRS)
   ============================================================ */
...
const App = {
  ...
  updateStats(){
    ...
    const st=$('#stat');
    if(st) st.innerHTML='학습 <b>'+c.seen+'</b> · 익힘 <b>'+c.learned+'</b> · 마스터 <b>'+c.mastered+'</b> · 복습대기 <b>'+c.due+'</b> · 오답 <b>'+c.weak+'</b>';
...
function buildScopeSelect(){
  ...
  let html='';
  for(let d=1; d<=DAYS; d++){ const a=(d-1)*getPerDay()+1, b=Math.min(d*getPerDay(),TOTAL); html+=`<option value="day:${d}">Day ${d} (${a}–${b})</option>`; }
  html+=`<option value="all">전체 ${TOTAL}단어</option>`;
  sel.innerHTML=html;
  sel.value=App.scope.type==='all'?'all':'day:'+App.scope.day;
  sel.onchange=()=>{ const v=sel.value; if(v==='all') App.scope={type:'all'}; else App.scope={type:'day', day:parseInt(v.split(':')[1],10)}; switchMode(App.state.mode||'flash'); };
}
function scopeLabel(){
...
document.addEventListener('DOMContentLoaded', ()=>{ bind(); showLogin(); });
