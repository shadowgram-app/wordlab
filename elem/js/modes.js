/* ============================================================
   WordLab 초등 · 학습 5모드 (이모지 카드)
   flash 플래시카드 · srs 간격복습 · quiz 4지선다 · dict 받아쓰기 · weak 오답 · me 내 현황
   ============================================================ */
function highlightEx(ex, w){
  try{ return ex.replace(new RegExp('\\b('+w+'\\w*)\\b','i'), '<b>$1</b>'); }catch(e){ return ex; }
}
function boxDots(w){
  const b=SRS.get(App.progress.known, w).box; let d='';
  for(let i=1;i<=SRS.MAX_BOX;i++) d+=`<span class="dot${i<=b?' on':''}"></span>`;
...
function cardInner(o){
  return `
    <div class="fcInner">
      ...
      <div class="face back">
        <div class="emoji sm">${o.emoji}</div>
...
function renderFlash(){
  ...
  const rateAdv=async(good)=>{ SRS.rate(App.progress.known,o.w,good); await App.save(); App.updateStats();
    if(App.state.cardIdx<list.length-1){App.state.cardIdx++; renderFlash();} else { emptyPanel('한 바퀴 완료! 🎉 다른 Day나 복습으로 넘어가 보세요.'); } };
  $('#good').onclick=(e)=>{ e.stopPropagation(); rateAdv(true); };
  $('#again').onclick=(e)=>{ e.stopPropagation(); rateAdv(false); };
}

/* ---------------- 간격 반복(SRS) ---------------- */
function startSrs(){ const q=SRS.dueList(App.progress.known, WORDS); App.state={mode:'srs', queue:q, done:0, total:q.length}; renderSrs(); }
function renderSrs(){
  const q=App.state.queue;
  if(!q.length){
    const nd=SRS.nextDue(App.progress.known, WORDS); const c=SRS.counts(App.progress.known, WORDS);
    const when=nd ? new Date(nd).toLocaleString('ko-KR',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}) : '아직 없음';
    emptyPanel(App.state.done>0
      ? `오늘 복습 완료! 🎉 ${App.state.done}개를 복습했어요.<br><span style="font-size:13px">다음 복습 예정: ${when}</span>`
      : `지금 복습할 단어가 없어요.<br><span style="font-size:13px">플래시카드로 새 단어를 배우면 복습 일정이 자동으로 잡혀요.<br>다음 복습 예정: ${when} · 익힘 ${c.learned}/${TOTAL}</span>`);
    return;
  }
  const o=wordObj(q[0]);
  $('#panel').innerHTML=`
    <div class="scopetag">🔁 복습 · ${App.state.done}/${App.state.total}</div>
    <div class="fc" id="fc">${cardInner(o)}</div>
    ${boxDots(o.w)}
    <div class="know"><button class="btn again" id="again">몰라요</button><button class="btn good" id="good">알아요 ✓</button></div>
    <div class="hint" style="position:static;margin-top:8px">단어를 보고 뜻을 떠올린 뒤 카드를 눌러 확인하세요</div>`;
  const fc=$('#fc'); fc.onclick=()=>fc.classList.toggle('flip');
  const rate=async(good)=>{ SRS.rate(App.progress.known,o.w,good); await App.save(); App.updateStats(); App.state.queue.shift(); App.state.done++; renderSrs(); };
...
function distractMeans(correct){
  const others=shuffle(WORDS.map(x=>x.mean)); const res=[];
  for(const m of others){ if(res.length>=3)break; if(m!==correct&&!res.includes(m))res.push(m); } return res;
}
function startQuiz(){ const list=App.scopeWords(); App.state={mode:'quiz', order:shuffle(list), idx:0, score:0, answered:false}; renderQuiz(); }
function renderQuiz(){
  const S=App.state;
...
async function finishQuiz(){
  ...
  $('#panel').innerHTML=`<div class="done"><h2>퀴즈 완료</h2><div class="score">${S.score} / ${S.order.length}</div>
    <p style="color:#6c6256">최고 점수 ${App.progress.quiz_score} · 틀린 단어는 오답·복습에 자동 반영돼요.</p>
...
/* ---------------- 받아쓰기 ---------------- */
function startDict(){ const list=App.scopeWords(); App.state={mode:'dict', order:shuffle(list), idx:0, score:0}; renderDict(); }
function renderDict(){
  const S=App.state;
  ...
  $('#panel').innerHTML=`
    ...
    <div class="dHint">그림과 뜻을 보고 영어 단어를 쓰세요</div>
...
async function finishDict(){
  ...
  $('#panel').innerHTML=`<div class="done"><h2>받아쓰기 완료</h2><div class="score">${S.score} / ${S.order.length}</div>
    <p style="color:#6c6256">최고 점수 ${App.progress.dict_score} · 철자가 틀린 단어는 오답·복습에 반영돼요.</p>
...
/* ---------------- 오답 ---------------- */
function startWeak(){ const q=SRS.weakList(App.progress.known, WORDS); App.state={mode:'weak', queue:q, done:0, total:q.length}; renderWeak(); }
function renderWeak(){
  const q=App.state.queue;
  if(!q.length){ emptyPanel(App.state.done>0 ? `오답 복습 완료! 🎉 ${App.state.done}개를 복습했어요.` : '오답 단어가 없어요! 잘하고 있어요. 👏'); return; }
  const o=wordObj(q[0]);
  $('#panel').innerHTML=`
    <div class="scopetag">🎯 오답 복습 · ${App.state.done}/${App.state.total}</div>
    <div class="fc" id="fc">${cardInner(o)}</div>
    ${boxDots(o.w)}
    <div class="know"><button class="btn again" id="again">아직 몰라요</button><button class="btn good" id="good">이제 알아요 ✓</button></div>`;
  const fc=$('#fc'); fc.onclick=()=>fc.classList.toggle('flip');
  const rate=async(good)=>{ SRS.rate(App.progress.known,o.w,good); await App.save(); App.updateStats(); App.state.queue.shift(); App.state.done++; renderWeak(); };
  $('#good').onclick=(e)=>{ e.stopPropagation(); rate(true); };
  $('#again').onclick=(e)=>{ e.stopPropagation(); rate(false); };
}
...
function renderMe(){
  ...
  const badges=[
    {i:'🌱', n:'첫걸음', e:c.seen>=1},
    {i:'📗', n:'Day 1 완료', e:days[0]&&days[0].learned===days[0].total},
    {i:'⭐', n:'첫 마스터', e:c.mastered>=1},
    {i:'🔥', n:'50 익힘', e:c.learned>=50},
    {i:'💯', n:'100 익힘', e:c.learned>=100},
    {i:'🎯', n:'오답 정복', e:c.weak===0&&c.seen>0},
    {i:'🏆', n:'전부 마스터', e:c.mastered>=TOTAL},
  ];
  const dayCells=days.map(p=>{
    const r=p.learned/p.total; const cls=p.mastered===p.total?'full':(r>=1?'learned':(r>0?'partial':''));
    return `<div class="daycell ${cls}" title="Day ${p.day}: 익힘 ${p.learned}/${p.total}"><span class="dn">${p.day}</span><span class="dc">${p.learned}/${p.total}</span></div>`;
  }).join('');
  const badgeCells=badges.map(b=>`<div class="badge2 ${b.e?'on':''}"><span class="bi">${b.i}</span><span class="bn">${b.n}</span></div>`).join('');
  $('#panel').innerHTML=`
    ...
    <div class="me-top">
      <div class="me-ring">
        <svg viewBox="0 0 120 120" width="130" height="130">
          ...
          <circle cx="60" cy="60" r="52" fill="none" stroke="url(#g)" stroke-width="12" stroke-linecap="round" stroke-dasharray="${dash.toFixed(1)} ${C.toFixed(1)}" transform="rotate(-90 60 60)"/>
          <defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="var(--accent2)"/><stop offset="1" stop-color="var(--gold)"/></linearGradient></defs>
        </svg>
